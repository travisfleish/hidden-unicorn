...
/** Public path to a player's full-size headshot, or null if we don't have one. */
...
/**
 * Face-centered square WebP avatars generated by scripts/resize-headshots.mjs.
 * The raw NBA PNG is a 1040x760 indexed, head-and-shoulders image; dropped into a
 * ~48px circle the face renders ~26px and the browser downscales ~15x in one
 * pass. These are pre-cropped to the face and resized with sharp/Lanczos, so the
 * face fills the avatar and the browser draws it near 1:1.
 */
export const HEADSHOT_VARIANT_WIDTHS = [96, 160, 320] as const
export type HeadshotVariantWidth = (typeof HEADSHOT_VARIANT_WIDTHS)[number]

/** Public path to one square avatar variant. */
export function headshotVariant(name: string, width: HeadshotVariantWidth): string | null {
  const id = headshotIds[name]
  return id ? `/headshots/r${width}/${id}.webp` : null
}

/** `{ src, srcSet }` for a responsive square avatar <img>; null if we have no headshot. */
export function headshotSrcSet(name: string): { src: string; srcSet: string } | null {
  const id = headshotIds[name]
  if (!id) return null
  return {
    src: `/headshots/r160/${id}.webp`,
    srcSet: HEADSHOT_VARIANT_WIDTHS.map((w) => `/headshots/r${w}/${id}.webp ${w}w`).join(', '),
  }
}
